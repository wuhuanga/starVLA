...
class _QWen_VL_Interface(nn.Module):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        if qwenvl_config.get("use_lora", False):
            ...
            if dual_lora:
                # Create two independent LoRA adapters for dual-branch frameworks
                model = get_peft_model(model, _make_lora_config(), adapter_name="posterior")
                model.add_adapter("prior", _make_lora_config())
                model.set_adapter("posterior")  # default to posterior
                self._has_dual_lora = True
                logger.info(
                    f"[DualLoRA] Applied dual LoRA to Qwen2.5-VL: r={lora_r}, "
                    f"alpha={lora_alpha}, adapters=['prior', 'posterior']"
                )
            else:
                model = get_peft_model(model, _make_lora_config())
                logger.info(
                    f"[LoRA] Applied LoRA to Qwen2.5-VL: r={lora_r}, alpha={lora_alpha}, "
